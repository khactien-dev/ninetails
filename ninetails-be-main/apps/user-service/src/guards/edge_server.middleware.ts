...
@Injectable()
export class EdgeServerReadMiddleware implements NestMiddleware {
...
@Injectable()
export class EdgeServerCreateMiddleware implements NestMiddleware {
  use(req: Request, res: Response, next: NextFunction) {
    if (req['auth'].role != EUserRole.ADMIN) {
      if (!req['auth'].permission) {
        throw new UnauthorizedException(
...
@Injectable()
export class EdgeServerUpdateMiddleware implements NestMiddleware {
  use(req: Request, res: Response, next: NextFunction) {
    if (req['auth'].role != EUserRole.ADMIN) {
      if (!req['auth'].permission) {
        throw new UnauthorizedException(
...
@Injectable()
export class EdgeServerDeleteMiddleware implements NestMiddleware {
  use(req: Request, res: Response, next: NextFunction) {
    if (req['auth'].role != EUserRole.ADMIN) {
      if (!req['auth'].permission) {
        throw new UnauthorizedException(
