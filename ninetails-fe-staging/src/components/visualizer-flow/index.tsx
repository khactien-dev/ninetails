...
import { useGetSchemaData } from '@/hooks/features/useRoute';
...
import { ICheckboxValues } from '../settings/routes/index.utils';
// import './Style';
import { InfoPopup, Markers } from './components';
import {
  calculateEdges,
...
const Loading = styled(BaseSkeleton.Node)`
  width: 100% !important;
...
const Visualizer: React.FC<VisualizerProps> = ({ checkboxValues }) => {
  ...
  const { data, isLoading } = useGetSchemaData();
  const currentDatabase = data?.data as DatabaseConfig;
  const database = useMemo(() => {
    return {
      ...currentDatabase,
      tables: (currentDatabase?.tables || []).filter((table) => checkboxValues[table.name]),
      edgeConfigs: (currentDatabase?.edgeConfigs || []).filter(
        (edge) => checkboxValues[edge.source.replace('schema.', '')]
      ),
    };
  }, [currentDatabase, checkboxValues]);

  return (
    <ReactFlowProvider>
      {isLoading ? <Loading active /> : <Flow currentDatabase={database || defaultDatabase} />}
    </ReactFlowProvider>
  );
};
