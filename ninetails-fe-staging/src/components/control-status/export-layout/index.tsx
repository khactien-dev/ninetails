...
import { getDriveState, minuteToTime } from '@/utils/control';
...
import * as Detail from '../vehicle-detail/index.styles';
import RouteHistory from '../vehicle-detail/route-history';
...
const ExportLayout = ({
  ...
  return (
    <div ref={contentRef}>
      ...
      <S.DateTitle>
        {lastUpdated
          ? dayjs(lastUpdated).format('YYYY년 MM월 DD일 (dddd)')
      ...
      <S.Space />
      <S.Condition>
        <S.Title open={isVisibleCondition}>운행모드</S.Title>
          ...
          <S.Collapse
            items={
              isSingleVehicle
                ? allStatus?.filter(
                    (item) =>
                      !!item.children.props.data?.find(
                        (i: any) => i.vehicle_number === params.vehicleNumber
                      )
                  )
                : allStatus
            }
            ...
            activeKey={allStatus
              ?.filter((item) => item.children.props.data.length)
              ?.map((item) => item.key)}
          />
        </S.Content>
      </S.Condition>
      ...
      {isSingleVehicle && (
        <Detail.ExportWrapper>
          ...
          <Detail.CardInfo>
            <Detail.CardTitle>
              ...
              <Detail.Value>
                ...
                <span>
                  {getDriveState(metricData?.latestDriveMetric?.drive_mode)?.name || '--'}
                </span>
              </Detail.Value>
            </Detail.CardTitle>
          ...
          <Detail.Wrapper>
            <RouteHistory history={metricData?.detailDriveMetric} height={0} />
            <Detail.Score>
              <strong>
            ...
            <Detail.Location>
              <figure />
              <div>
                <p>총 이동 거리: {metricData?.totals?.trip_distance || '--'} km</p>
                <span>총 시간: {minuteToTime(metricData?.totals?.trip_time) || '--'}</span>
              </div>
            </Detail.Location>
          </Detail.Wrapper>
        </Detail.ExportWrapper>
