import { getGuideCode } from '@/api/route/guide-code';
...
export default function useGuideRoute() {
  ...
  const deleteGuide = useDeleteManyGuideCode();
  const createGuide = useCreateGuideCode();
  const importGuide = useImportGuideCode();
  const exportData = useExportData();
  const updateGuide = useUpdateGuideCode();
  ...
  const handleGetData = async (newParams: RouteParams, refetch: boolean) => {
    ...
    try {
      setIsLoading(true);
      const data = await getGuideCode(updateParams);
      setIsLoading(false);
  ...
  const handleSave = async () => {
    ...
    if (isDuplicateCode) {
      notification.error({ message: 'Guide code is exists' });
      return;
    }
    ...
    try {
      ...
      notification.success({ message: 'Guide code saved successfully' });
    } catch (error) {
      setLoadingSaveData(false);
    }
  };
  ...
  const handleExport = (values: any) => {
    ...
    exportData.mutate(
      { table: TABLE_NAME.GUIDE_CODE, type: values.fileType === 'csv' ? 'csv' : 'excel' },
      {
        onSuccess: (data) => {
