import { getSection } from '@/api/route/section';
...
import { validateFloatNumber, validateIntNumber } from '@/utils';
...
export default function useSection() {
  ...
  const deleteSection = useDeleteManySection();
  const createSection = useCreateSection();
  const importSection = useImportSection();
  ...
  const isLoadingTable =
    isFetching || loadingSaveData || deleteSection.isPending || importSection.isPending;
  ...
  const handleGetData = async (newParams: RouteParams, refetch: boolean) => {
    ...
    try {
      setIsLoading(true);
      const data = await getSection(updateParams);
      setIsLoading(false);
  ...
  const handleSave = async () => {
    ...
    if (isDuplicateName) {
      notification.error({ message: 'Section name is exists' });
      return;
    }

    deleteRowKeys.length > 0 && (await handleDeleteSection());
    ...
    const createPromise = () => createSection.mutateAsync({ data: createPayload });
    const updatePromise = () => updateSection.mutateAsync(updatePayload);
    ...
    try {
      ...
      notification.success({ message: 'The section saved successfully' });
    } catch (error) {
      setLoadingSaveData(false);
    }
  };
  ...
  const handleDeleteSection = async () => {
    ...
    listRowKeys.length > 0 && (await deleteSection.mutateAsync(listRowKeys));
  ...
  const handleImportSection = () => {
    if (selectedFile) {
      importSection.mutate(selectedFile, {
        onSuccess: () => {
          refetch();
  ...
  const handleExport = (values: any) => {
    ...
    exportData.mutate(
      { table: TABLE_NAME.SECTION, type: values.fileType === 'csv' ? 'csv' : 'excel' },
      {
        onSuccess: (data) => {
