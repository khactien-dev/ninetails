import { getCoreSection } from '@/api/route/core-section';
...
import {
  useCreateCoreSection,
  useDeleteManyCoreSection,
  useExportData,
  useImportCoreSection,
  useUpdateCoreSection,
} from '@/hooks/features/useRoute';
import { useFeedback } from '@/hooks/useFeedback';
import { usePermissions } from '@/hooks/usePermissions';
import { ButtonItem, ColumnOption, RouteParams } from '@/interfaces';
import { validateIntNumber } from '@/utils';
...
export default function useCoreSection() {
  ...
  const deleteCoreSection = useDeleteManyCoreSection();
  const createCoreSection = useCreateCoreSection();
  const importCoreSection = useImportCoreSection();
  const exportData = useExportData();
  const updateCoreSection = useUpdateCoreSection();
  ...
  const isLoadingTable =
    isFetching || loadingSaveData || deleteCoreSection.isPending || importCoreSection.isPending;
  ...
  const handleGetData = async (newParams: RouteParams, refetch: boolean) => {
    ...
    try {
      setIsLoading(true);
      const data = await getCoreSection(updateParams);
      setIsLoading(false);
  ...
  const handleSave = async () => {
    ...
    const isInvalidRow = filterDataTable.some(
      (row) =>
        ...
        row.segment_id === initialValuesDefault.segment_id ||
        row.type === initialValuesDefault.type ||
        row.point_index === initialValuesDefault.point_index ||
        row.route_type === initialValuesDefault.route_type
    );
    ...
    if (isDuplicateName) {
      notification.error({ message: 'core section name is exists' });
      return;
    }

    deleteRowKeys.length > 0 && (await handleDeleteCoreSection());
    ...
    const createPromise = () => createCoreSection.mutateAsync({ data: createPayload });
    const updatePromise = () => updateCoreSection.mutateAsync(updatePayload);
    ...
    try {
      ...
      notification.success({ message: 'Core section saved successfully' });
    } catch (error) {
      setLoadingSaveData(false);
    }
  };
      ...
      render: (_, record) => {
        return (
          <ContentEditable
            ...
            required={true}
            isRefresh={isRefresh}
            data={record.segment_id}
            validate={validateIntNumber}
            handleConfirm={(value: string, isColumnError: boolean) => {
              validateColumn('segment_id', isColumnError, record.key);
              if (isColumnError) return;
              handleChangeColumn(Number(value), 'segment_id', record.key);
            }}
          />
        );
      },
      ...
      render: (_, record) => {
        return (
          <ContentEditable
            ...
            required={true}
            isRefresh={isRefresh}
            data={record.route_type}
            handleConfirm={(value: string, isColumnError: boolean) => {
              validateColumn('route_type', isColumnError, record.key);
              if (isColumnError) return;
              handleChangeColumn(value, 'route_type', record.key);
            }}
          />
        );
      },
  ...
  const handleDeleteCoreSection = async () => {
    ...
    listRowKeys.length > 0 && (await deleteCoreSection.mutateAsync(listRowKeys));
  ...
  const handleImportCoreSection = () => {
    if (selectedFile) {
      importCoreSection.mutate(selectedFile, {
        onSuccess: () => {
          refetch();
  ...
  const handleExport = (values: any) => {
    ...
    exportData.mutate(
      { table: TABLE_NAME.CORE_SECTION, type: values.fileType === 'csv' ? 'csv' : 'excel' },
      {
        onSuccess: (data) => {
  ...
  return {
    ...
    handleExport,
    onChangeHeader,
    handleDeleteCoreSection,
    handleImportCoreSection,
    handleFileChange,
    setSelectedFile,
    handleRemoveFile,
    handleQuery,
