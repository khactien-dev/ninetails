import { getSegmentsRoute } from '@/api/route';
...
import {
  useCreateSegmentRoute,
  useDeleteManySegmentRoute,
  useExportData,
  useImportSegmentRoute,
  useUpdateSegmentRoute,
...
export default function useSegmentsRoute() {
  ...
  const deleteSegmentRoute = useDeleteManySegmentRoute();
  const createSegmentRoute = useCreateSegmentRoute();
  const importSegmentRoute = useImportSegmentRoute();
  const exportData = useExportData();
  const updateSegmentRoute = useUpdateSegmentRoute();
  ...
  const isLoadingTable =
    isFetching || loadingSaveData || deleteSegmentRoute.isPending || importSegmentRoute.isPending;
  ...
  const handleGetData = async (newParams: RouteParams, refetch: boolean) => {
    ...
    try {
      setIsLoading(true);
      const data = await getSegmentsRoute(updateParams);
      setIsLoading(false);
  ...
  const handleSave = async () => {
    ...
    const isInvalidRow = filterDataTable.some(
      (row) =>
        ...
        row.section_id === initialValuesDefault.section_id
    );
    ...
    deleteRowKeys.length > 0 && (await handleDeleteSegmentRoute());
    ...
    const createPromise = () => createSegmentRoute.mutateAsync({ data: createPayload });
    const updatePromise = () => updateSegmentRoute.mutateAsync(updatePayload);
    ...
    try {
      ...
      notification.success({ message: 'The segment route saved successfully' });
    } catch (error) {
      setLoadingSaveData(false);
    }
  };
      ...
      render: (_, record) => {
        return (
          <ContentEditable
            ...
            required={true}
            isRefresh={isRefresh}
            data={record.section_id}
            validate={validateIntNumber}
            handleConfirm={(value: string, isColumnError: boolean) => {
              validateColumn('section_id', isColumnError, record.key);
              if (isColumnError) return;
              handleChangeColumn(Number(value), 'section_id', record.key);
            }}
          />
        );
      },
  ...
  const handleDeleteSegmentRoute = async () => {
    ...
    listRowKeys.length > 0 && (await deleteSegmentRoute.mutateAsync(listRowKeys));
  ...
  const handleImportSegmentRoute = () => {
    if (selectedFile) {
      importSegmentRoute.mutate(selectedFile, {
        onSuccess: () => {
          refetch();
  ...
  const handleExport = (values: any) => {
    ...
    exportData.mutate(
      { table: TABLE_NAME.SEGMENT_ROUTE, type: values.fileType === 'csv' ? 'csv' : 'excel' },
      {
        onSuccess: (data) => {
  ...
  return {
    ...
    handleExport,
    onChangeHeader,
    handleDeleteSegmentRoute,
    handleImportSegmentRoute,
    handleFileChange,
    setSelectedFile,
    handleRemoveFile,
    handleQuery,
