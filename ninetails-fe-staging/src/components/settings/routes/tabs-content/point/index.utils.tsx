import { getPoint } from '@/api/route/point';
...
export default function usePoint() {
  ...
  const deletePoint = useDeleteManyPoint();
  const createPoint = useCreatePoint();
  const importPoint = useImportPoint();
  ...
  const isLoadingTable =
    isFetching || loadingSaveData || deletePoint.isPending || importPoint.isPending;
  ...
  const handleGetData = async (newParams: RouteParams, refetch: boolean) => {
    ...
    try {
      setIsLoading(true);
      const data = await getPoint(updateParams);
      setIsLoading(false);
  ...
  const handleSave = async () => {
    ...
    if (isDuplicateName) {
      notification.error({ message: 'Point name is exists' });
      return;
    }

    deleteRowKeys.length > 0 && (await handleDeletePoint());
    ...
    const createPromise = () => createPoint.mutateAsync({ data: createPayload });
    const updatePromise = () => updatePoint.mutateAsync(updatePayload);
    ...
    try {
      ...
      notification.success({ message: 'The point saved successfully' });
    } catch (error) {
      setLoadingSaveData(false);
    }
  };
  ...
  const handleDeletePoint = async () => {
    ...
    listRowKeys.length > 0 && (await deletePoint.mutateAsync(listRowKeys));
  ...
  const handleImportPoint = () => {
    if (selectedFile) {
      importPoint.mutate(selectedFile, {
        onSuccess: () => {
          refetch();
  ...
  const handleExport = (values: any) => {
    ...
    exportData.mutate(
      { table: TABLE_NAME.POINT, type: values.fileType === 'csv' ? 'csv' : 'excel' },
      {
        onSuccess: (data) => {
