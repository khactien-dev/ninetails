import { getListSegment } from '@/api/route';
...
export default function useSegments() {
  ...
  const deleteSegment = useDeleteManySegment();
  const createSegment = useCreateSegment();
  const importSegment = useImportSegment();
  ...
  const isLoadingTable =
    isFetching || loadingSaveData || deleteSegment.isPending || importSegment.isPending;
  ...
  const handleGetData = async (newParams: RouteParams, refetch: boolean) => {
    ...
    try {
      setIsLoading(true);
      const data = await getListSegment(updateParams);
      setIsLoading(false);

      const table = data?.data?.data.map((item) => ({
        ...item,
        key: String(item.id),
        must_pass: item.must_pass.toString().toUpperCase(),
        manual_collect: item.manual_collect.toString().toUpperCase(),
      }));
  ...
  const handleSave = async () => {
    ...
    const isInvalidRow = filterDataTable.some(
      (row) =>
        row.segment_line === initialValuesDefault.segment_line ||
        row.distance === initialValuesDefault.distance ||
        row.name === initialValuesDefault.name ||
        row.congestion === initialValuesDefault.congestion ||
    ...
    if (isDuplicateName) {
      notification.error({ message: 'Segment name is exists' });
      return;
    }

    deleteRowKeys.length > 0 && (await handleDeleteSegment());
    ...
    createPayload.forEach((item: any) => {
      delete item.id;
      delete item.key;
      item.must_pass = item.must_pass === 'TRUE' || item.must_pass === 'true' ? true : false;
      item.manual_collect =
        item.manual_collect === 'TRUE' || item.manual_collect === 'true' ? true : false;
    });
    ...
    const createPromise = () => createSegment.mutateAsync({ data: createPayload });
    const updatePromise = () => updateSegment.mutateAsync(updatePayload);
    ...
    try {
      ...
      if (createPayload.length > 0 && promise1?.status === 'fulfilled') {
        ...
        const updateData = updateRow.map((item) => ({
          ...item,
          must_pass: item.must_pass.toString().toUpperCase(),
          manual_collect: item.manual_collect.toString().toUpperCase(),
        }));
        setDataTable([...createData, ...updateData]);
      ...
      notification.success({ message: 'The segment saved successfully' });
    } catch (error) {
      setLoadingSaveData(false);
    }
  };
  ...
  const columns: ColumnsType<any> = [
      ...
      render: (_, record) => {
        return (
          <ContentEditable
            ...
            required={true}
            isRefresh={isRefresh}
            data={record.segment_line ? JSON.stringify(record.segment_line) : ''}
            validate={checkLinestringGeometry}
            handleConfirm={(value: string, isColumnError: boolean) => {
              validateColumn('segment_line', isColumnError, record.key);
              if (isColumnError) return;
              handleChangeColumn(JSON.parse(value), 'segment_line', record.key);
            }}
          />
        );
      },
      ...
      render: (_, record) => {
        return (
          <ContentEditable
            ...
            required={true}
            isRefresh={isRefresh}
            data={record.congestion}
            validate={validateFloatNumber}
            handleConfirm={(value: string, isColumnError: boolean) => {
              validateColumn('congestion', isColumnError, record.key);
              if (isColumnError) return;
              handleChangeColumn(Number(value), 'congestion', record.key);
            }}
          />
        );
      },
      ...
      render: (_, record) => {
        return (
          <ContentEditable
            ...
            required={true}
            isRefresh={isRefresh}
            data={record.must_pass}
            validate={checkBoolean}
            handleConfirm={(value: string, isColumnError: boolean) => {
              validateColumn('must_pass', isColumnError, record.key);
              if (isColumnError) return;
              handleChangeColumn(value, 'must_pass', record.key);
            }}
          />
        );
      },
    ...
    {
      title: 'manual_collect',
      key: 'manual_collect',
      dataIndex: 'manual_collect',
      render: (_, record) => {
        return (
          <ContentEditable
            activeKey={record.key}
            ...
            required={true}
            isRefresh={isRefresh}
            data={record.manual_collect}
            validate={checkBoolean}
            handleConfirm={(value: string, isColumnError: boolean) => {
              validateColumn('manual_collect', isColumnError, record.key);
              if (isColumnError) return;
              handleChangeColumn(value, 'manual_collect', record.key);
            }}
          />
        );
      },
  ...
  const handleDeleteSegment = async () => {
    ...
    listRowKeys.length > 0 && (await deleteSegment.mutateAsync(listRowKeys));
  ...
  const handleImportSegment = () => {
    if (selectedFile) {
      importSegment.mutate(selectedFile, {
        onSuccess: () => {
          refetch();
  ...
  const handleExport = (values: any) => {
    ...
    exportData.mutate(
      { table: TABLE_NAME.SEGMENT, type: values.fileType === 'csv' ? 'csv' : 'excel' },
      {
        onSuccess: (data) => {
