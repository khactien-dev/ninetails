import { getMetadata } from '@/api/route';
...
import { checkTimestamp, validateIntNumber } from '@/utils';
...
export default function useMetadata() {
  ...
  const deleteMetadata = useDeleteManyMetadata();
  ...
  const isLoadingTable =
    isFetching || loadingSaveData || deleteMetadata.isPending || importMetadata.isPending;
  ...
  const handleGetData = async (newParams: RouteParams, refetch: boolean) => {
    ...
    try {
      setIsLoading(true);
      const data = await getMetadata(updateParams);
      setIsLoading(false);
  ...
  const handleSave = async () => {
    ...
    const isInvalidRow = filterDataTable.some(
      (row) =>
        row.table_name === initialValuesDefault.table_name ||
        row.version === initialValuesDefault.version ||
        row.updated_by === initialValuesDefault.updated_by ||
        row.update_time === initialValuesDefault.update_time
    );
    ...
    deleteRowKeys.length > 0 && (await handleDeleteMetadata());
    ...
    const createPromise = () => createMetadata.mutateAsync({ data: createPayload });
    const updatePromise = () => updateMetadata.mutateAsync(updatePayload);
        ...
        return (
          <ContentEditable
            ...
            handleConfirm={(value: string, isColumnError: boolean) => {
              validateColumn('table_name', isColumnError, record.key);
              if (isColumnError) return;
              handleChangeColumn(value, 'table_name', record.key);
            }}
          />
        );
      },
      ...
      render: (_, record) => {
        return (
          <ContentEditable
            ...
            handleConfirm={(value: string, isColumnError: boolean) => {
              validateColumn('version', isColumnError, record.key);
              if (isColumnError) return;
              handleChangeColumn(Number(value), 'version', record.key);
            }}
          />
        );
      },
      ...
      render: (_, record) => {
        return (
          <ContentEditable
            ...
            handleConfirm={(value: string, isColumnError: boolean) => {
              validateColumn('updated_by', isColumnError, record.key);
              if (isColumnError) return;
              handleChangeColumn(value, 'updated_by', record.key);
            }}
          />
        );
      },
      ...
      render: (_, record) => {
        return (
          <ContentEditable
            ...
            disabled={!isSaveAble}
            required={true}
            data={record.update_time}
            isRefresh={isRefresh}
            validate={checkTimestamp}
            handleConfirm={(value: string, isColumnError: boolean) => {
              validateColumn('update_time', isColumnError, record.key);
              if (isColumnError) return;
              handleChangeColumn(value, 'update_time', record.key);
            }}
          />
        );
      },
  ...
  const handleDeleteMetadata = async () => {
    ...
    listRowKeys.length > 0 && (await deleteMetadata.mutateAsync(listRowKeys));
  ...
  const handleImportMetadata = () => {
    if (selectedFile) {
      importMetadata.mutate(selectedFile, {
        onSuccess: () => {
          refetch();
  ...
  const handleExport = (values: any) => {
    ...
    exportData.mutate(
      { table: TABLE_NAME.METADATA, type: values.fileType === 'csv' ? 'csv' : 'excel' },
      {
        onSuccess: (data) => {
