import { getCongestionCode } from '@/api/route';
...
import {
  useCreateCongestionCode,
  useDeleteManyCongestionCode,
  useExportData,
  useImportCongestionCode,
  useUpdateCongestionCode,
} from '@/hooks/features/useRoute';
import { useFeedback } from '@/hooks/useFeedback';
import { usePermissions } from '@/hooks/usePermissions';
import { ButtonItem, RouteParams } from '@/interfaces';
import { validateFloatNumber } from '@/utils';
...
export default function useCongestionCode() {
  ...
  const deleteCongestionCode = useDeleteManyCongestionCode();
  const createCongestionCode = useCreateCongestionCode();
  const importCongestionCode = useImportCongestionCode();
  const exportData = useExportData();
  const updateCongestionCode = useUpdateCongestionCode();
  ...
  const isLoadingTable =
    isFetching ||
    loadingSaveData ||
    deleteCongestionCode.isPending ||
    importCongestionCode.isPending;
  ...
  const handleGetData = async (newParams: RouteParams, refetch: boolean) => {
    ...
    try {
      setIsLoading(true);
      const data = await getCongestionCode(updateParams);
      setIsLoading(false);
  ...
  const handleSave = async () => {
    ...
    const isInvalidRow = filterDataTable.some(
      (row) =>
        row.code === initialValuesDefault.code ||
        row.description === initialValuesDefault.description
    );

    const isDuplicateCode = filterDataTable.some(
      (item, index) => filterDataTable.findIndex((obj) => obj.code === item.code) !== index
    );
    ...
    if (isDuplicateCode) {
      notification.error({ message: 'code must be unique' });
      return;
    }

    deleteRowKeys.length > 0 && (await handleDeleteCongestionCode());
    ...
    const createPromise = () => createCongestionCode.mutateAsync({ data: createPayload });
    const updatePromise = () => updateCongestionCode.mutateAsync(updatePayload);
    ...
    try {
      ...
      notification.success({ message: 'Congestion codes saved successfully' });
    } catch (error) {
      setLoadingSaveData(false);
    }
  };
        ...
        return (
          <ContentEditable
            ...
            handleConfirm={(value: string, isColumnError: boolean) => {
              validateColumn('code', isColumnError, record.key);
              if (isColumnError) return;
              handleChangeColumn(Number(value), 'code', record.key);
            }}
          />
        );
      },
      ...
      render: (_, record) => {
        return (
          <ContentEditable
            ...
            handleConfirm={(value: string, isColumnError: boolean) => {
              validateColumn('description', isColumnError, record.key);
              if (isColumnError) return;
              handleChangeColumn(value, 'description', record.key);
            }}
          />
        );
      },
  ...
  const handleDeleteCongestionCode = async () => {
    ...
    listRowKeys.length > 0 && (await deleteCongestionCode.mutateAsync(listRowKeys));
  ...
  const handleImportCongestionCode = () => {
    if (selectedFile) {
      importCongestionCode.mutate(selectedFile, {
        onSuccess: () => {
          refetch();
  ...
  const handleExport = (values: any) => {
    ...
    exportData.mutate(
      { table: TABLE_NAME.CONGESTION_CODE, type: values.fileType === 'csv' ? 'csv' : 'excel' },
      {
        onSuccess: (data) => {
  ...
  return {
    ...
    handleExport,
    onChangeHeader,
    handleDeleteCongestionCode,
    handleImportCongestionCode,
    handleFileChange,
    setSelectedFile,
    handleRemoveFile,
    handleQuery,
